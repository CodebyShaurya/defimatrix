  ...
  return (
    <AnimatePresence mode="wait">
      ...
      <motion.div
        ...
        className={cn("fixed top-8 z-[5000] flex w-full items-center justify-evenly px-4 py-2", className)}
      >
        {navItems.map((navItem: any, idx: number) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative flex items-center text-base font-medium uppercase tracking-wider text-neutral-500 hover:text-white hover:underline  hover:underline-offset-4 2xl:text-lg",
            )}
          >
        ...
        <a href="/" className="">
          <span className="sr-only">DefiMatrix</span>
          <img className="h-8 w-auto md:h-12 2xl:h-16" src="/defimatrix-logo.svg" alt="defimatrix-logo" />
        </a>
        {navItems2.map((navItem2: any, idx: number) => (
          <Link
            key={`link=${idx}`}
            href={navItem2.link}
            className={cn(
          ...
          >
            <span className="block sm:hidden">{navItem2.icon}</span>
            <span className="hidden sm:block">{navItem2.name}</span>
          </Link>
        ))}
        {/* <a href="#contact-us"
          onClick={() => {
          ...
          className="relative rounded-full border border-white/[0.2] px-4 py-2 text-base 2xl:text-lg font-semibold uppercase tracking-tight text-white hover:shadow hover:shadow-blue-500"
        >
